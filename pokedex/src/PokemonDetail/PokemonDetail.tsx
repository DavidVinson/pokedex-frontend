...
function PokemonDetail() {
    ...
    return (
        ...
        <Flex className="profile">
            ...
            <Flex>
                <IconButton aria-label="left-arrow" icon={<FaArrowLeft color="white" />} isRound={true} size='lg' bgColor="teal.500" onClick={() => history.push(`/page/${currentPage}`)} />
            ...
            <div>
                ...
                <h2>{pokemonDetail?.name} #{pokeID}</h2>

                {pokemonDetail?.types.map((type) => <p>{type.toUpperCase()} </p>)}
                <Center><img src={pokemonDetail?.image} alt={pokemonDetail?.name} /></Center>
